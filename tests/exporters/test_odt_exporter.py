...
class TestODTExporterBranding:
    """Tests that exported ODT documents do not inject application branding."""
    ...
    def test_output_has_no_application_attribution(self, exporter, simple_markdown):
        """The generated document must contain only report content/metadata."""
        import io
        import zipfile
        ...
        assert "Generated by" not in content_xml
        assert "LDR - Local Deep Research" not in content_xml
        assert "Open Source AI Research Assistant" not in content_xml
        assert "github.com/LearningCircuit/local-deep-research" not in content_xml
