...
class TestPDFServiceMarkdownToHtml:
    ...
    def test_excludes_application_attribution(self, service, simple_markdown):
        """Exported PDF HTML must not inject application branding."""
        html = service._markdown_to_html(simple_markdown)

        assert "Generated by" not in html
        assert "LDR - Local Deep Research" not in html
        assert "Open Source AI Research Assistant" not in html
        assert "github.com/LearningCircuit/local-deep-research" not in html
